...
class University extends React.Component {
  ...
  render(props) {
    ...
    return (
      ...
      >
        <div
          className="university_card"
          style={{ overflow: items.length === 0 ? "hidden" : null }}
        >
          {items.length === 0 ? (
            <span id="msg">No Data Available</span>
          ) : (
            items.map((item) => <ChoiceUn title={item} s_name={s_name} />)
          )}
        </div>
      </motion.div>
    );
